...
CREATE TABLE `exam`  (
  ...
  `endDate` datetime NOT NULL COMMENT '考试结束时间',
  `status` tinyint NOT NULL DEFAULT 000 COMMENT '考试状态（0 - 进行中，1 - 完成）',
  `category` tinyint NOT NULL DEFAULT 000 COMMENT '考试类别（0-教师结构化面试）',
...
CREATE TABLE `exam-record`  (
  ...
  `user_answer` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '用户面试回答',
  `user_audio_url` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '语音文件静态存储地址',
  `startDate` datetime NOT NULL COMMENT '创建时间',
  `endDate` datetime NOT NULL COMMENT '保存时间',
...
CREATE TABLE `exam-result`  (
  `id` int NOT NULL COMMENT '考试结果id',
  `exam_id` int NOT NULL COMMENT '关联考试id',
  `score` tinyint(3) UNSIGNED ZEROFILL NOT NULL COMMENT '考试结果分数',
  `ability` char(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '能力评分（JSON）',
...
CREATE TABLE `knowlege-category`  (
  `id` int NOT NULL COMMENT '知识条目id',
...
CREATE TABLE `question`  (
  ...
  `category` tinyint(3) UNSIGNED ZEROFILL NOT NULL DEFAULT 000 COMMENT '题目大类(0 - 教师考试，1 - 公务员考试)',
